...
export default function DevotionalHighlight() {
  ...
  return (
    <section
      id="devocional"
      className="py-24 bg-navy-gradient text-white"
      aria-label="Devocional destacado"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <SectionHeader
              ...
              subtitle="Nuestros colaboradores comparten reflexiones bíblicas para inspirarte y fortalecerte en la fe."
              light
              centered={false}
            />
            <Link
              href="/devocionales"
              className="mt-8 inline-flex items-center gap-2 text-gold-300 hover:text-gold-200 font-semibold text-sm transition-colors"
            >
              Explorar todos los devocionales <ArrowRight size={14} />
            </Link>
          </div>

          {/* Featured devotional card */}
          <div>
            <Link href={`/devocionales/${devotional.slug}`} className="group block">
              <article className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-8 hover:bg-white/15 hover:border-gold-400/40 transition-all duration-300">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {devotional.tags.slice(0, 2).map((tag) => (
                    <Badge
                      key={tag.id}
                      variant="gold"
                      className="bg-gold-500/20 border-gold-400/40 text-gold-300"
                    >
                      {tag.name}
                    </Badge>
                  ))}
                ...
                <h3 className="font-serif text-2xl font-bold text-white mb-3 group-hover:text-gold-200 transition-colors leading-tight">
                  {devotional.title}
                </h3>
