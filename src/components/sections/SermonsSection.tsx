...
export default function SermonsSection({ dict }: { dict: SermonsDict }) {
  const sermons = getRecentSermons(4);
  const [featured, ...rest] = sermons;

  return (
    <section id="predicaciones" className="py-24 bg-warm-50" aria-label="Predicaciones recientes">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-12">
          <SectionHeader eyebrow={dict.eyebrow} title={dict.title} subtitle={dict.subtitle} centered={false} />
          <a href={youtubeConfig.channelUrl} target="_blank" rel="noopener noreferrer" className="text-gold-600 hover:text-gold-700 font-semibold text-sm flex items-center gap-1 transition-colors shrink-0">
          ...
          {featured && (
            ...
            >
              <div className="relative aspect-video bg-navy-900">
                <Image src={featured.thumbnailUrl} alt={featured.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 1024px) 100vw, 60vw" />
                ...
                {featured.series && (
                  <div className="absolute top-3 left-3">
                    <Badge variant="navy" className="bg-navy-800/80 text-white backdrop-blur-sm">{featured.series}</Badge>
                  </div>
                )}
              </div>
              <div className="p-6">
                ...
                <h3 className="font-serif text-2xl font-bold text-navy-900 group-hover:text-navy-700 transition-colors mb-2">{featured.title}</h3>
                <p className="text-slate-500 text-sm leading-relaxed mb-3">{featured.excerpt}</p>
                <div className="flex items-center gap-3 text-xs text-warm-500">
                  <span className="flex items-center gap-1"><BookOpen size={12} />{featured.scriptureReference}</span>
                  {featured.durationMinutes && <span className="flex items-center gap-1"><Clock size={12} />{featured.durationMinutes} min</span>}
                </div>
              </div>
            </a>
          )}
            ...
            {rest.map((sermon) => (
              ...
              >
                <div className="relative w-24 h-16 rounded-lg overflow-hidden shrink-0 bg-navy-900">
                  <Image src={sermon.thumbnailUrl} alt={sermon.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="96px" />
                  <div className="absolute inset-0 flex items-center justify-center bg-navy-950/30">
                    <Play size={14} className="text-white fill-white" />
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-gold-600 font-semibold mb-1">{formatDate(sermon.date, { day: "numeric", month: "short" })}</p>
                  <h4 className="font-serif font-bold text-navy-900 text-sm leading-snug group-hover:text-navy-700 transition-colors line-clamp-2">{sermon.title}</h4>
